                ...
                <div class="row">
                    <div class="card overflow-hidden">
                        <div class="card-header bg-vanblue p-2">
                            <h3 class="cw">COMPLAINT/FEEDBACK
                                <?php if ($_SESSION['SESS_USER_TYPE'] != 'admin') { ?>
                                    <a href="create-complaint.php" class="btn btn-sm btn-primary pull-right">ADD NEW</a>
                                <?php } ?>
                            </h3>
                        </div>
                        <div class="card-body p-2"><br>
                            <div class="list-group">
                                <?php
                                $qry = "SELECT * FROM complaints WHERE created_by = '$user'";
                                if ($_SESSION['SESS_USER_TYPE'] == 'admin') {
                                    $qry = "SELECT * FROM complaints";
                                }
                                $qry_ncount = $db->prepare($qry);
                                $qry_ncount->execute();
                                for ($i = 0; $rows = $qry_ncount->fetch(); $i++) {
                                ?>
                                    <div class="list-group-item col-md-12">
                                        <h4 class="list-group-item-heading"><?php echo strtoupper($rows['subject']) . "<br>" . ucfirst($rows['description']) ?></h4>
                                        <p class="list-group-item-text">
                                            <?php
                                            if ($_SESSION['SESS_USER_TYPE'] == 'admin') {
                                                $stmt = $db->prepare("SELECT * FROM users WHERE token = ? LIMIT 0,1");
                                                $stmt->bindParam(1, $rows['created_by']);
                                                $stmt->execute();
                                                $user_rows = $stmt->fetch(PDO::FETCH_ASSOC);
                                                echo "By " . ucwords($user_rows['name']) . " (" . $user_rows['contact_no'] . ") @ " . time_convert($rows['created_at']);
                                            } else {
                                                echo time_convert($rows['created_at']);
                                            }
                                            ?>
                                        </p>
                                        <?php if ($_SESSION['SESS_USER_TYPE'] == 'admin') { ?>
                                            ...
                                            <div class="col-12 mt-2">
                                                <a class="btn btn-danger mb-2" style="color:#fff" href="reply-complaint.php?id=<?php echo $rows['token'] ?>">Send/Edit Reply</a>
                                                <h4 class="text-info"><?php echo ucfirst($rows['reply']); ?></h4>
                                            </div>
                                        <?php } elseif ($rows['reply'] != null) { ?>
                                            <div class="clearfix"></div>
                                            <div class="col-12 mt-2">
                                                <button class="btn btn-danger mb-2" style="color:#fff">Admin Reply</button>
                                                <h4 class="text-info"><?php echo ucfirst($rows['reply']); ?></h4>
                                            </div>
                                        <?php } ?>
                                    </div>
                        ...
                        <div class="card-footer p-2">
                            <a href="myaccount.php" class="btn btn-primary btn-lg cw pull-left">DASHBOARD</a>
                            <div class="clearfix"></div>
                        </div>
                    </div>
                </div>
