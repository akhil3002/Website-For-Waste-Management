...
$qry = $db->prepare("SELECT * FROM complaints WHERE token = '$token'");
$qry->execute();
$rows = $qry->fetch();

$err = '';
if (isset($_POST['create'], $_POST['token'], $_POST['reply'])) {
    $token = $_POST['token'];
    $reply = $_POST['reply'];

    $db->prepare("UPDATE complaints SET reply = '$reply' WHERE token = '$token'")->execute();
                ...
                <div class="row">
                    <div class="card overflow-hidden">
                        <div class="card-header bg-vanblue p-2">
                            <h3 class="cw">
                                <a href="complaints.php" class="mr-2"><i class="fa fa-arrow-left"></i></a>
                                REPLY COMPLAINT/FEEDBACK
                            </h3>
                        </div>
                        <div class="contact-middle" style="padding: 0;">
                            <form action="reply-complaint.php" method="post" enctype="multipart/form-data" autocomplete="off">
                                <input type="hidden" name="token" value="<?php echo $token ?>">
                                <div class="card-body p-2"><br>
                                    <div class="row">
                                        <div class="col-md-12 mb-4">
                                            <input type="text" name="subject" class="form-control form-control-lg" placeholder="subject" required readonly value="<?php echo $rows['subject'] ?>">
                                        </div>
                                        <div class="col-md-12 mb-4">
                                            <textarea name="description" class="form-control form-control-lg" placeholder="complaint/feedback in detail" required readonly><?php echo $rows['description'] ?></textarea>
                                        </div>
                                        <div class="col-md-12 mb-4">
                                            <textarea name="reply" class="form-control form-control-lg" placeholder="reply for this complaint/feedback" required autofocus><?php echo $rows['reply'] ?></textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="card-footer">
                                    <button type="button" onclick="location.href = 'complaints.php';" class="btn btn-danger btn-lg pull-left mr-2">CANCEL</button>
                                    <button type="submit" name="create" class="btn btn-success btn-lg pull-right">SEND</button>
