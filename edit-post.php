...
$err = '';
$token = $_GET['token'];
$qry = $db->prepare("SELECT * FROM posts WHERE token = '$token' AND created_by = '$user' AND closed = 0");
$qry->execute();
if ($qry->rowCount() == 0) {
...
if (isset($_POST['update_post'], $_POST['title'], $_POST['description'])) {
    $title = $_POST['title'];
    $description = $_POST['description'];
    $address = $_POST['address'];

    $db->prepare("UPDATE posts SET title = '$title', description = '$description', address = '$address' WHERE token = '$token'")->execute();

    header("location:edit-post.php?token=$token");
}
if (isset($_POST['update_image'])) {
    ...
    $db->prepare("UPDATE posts SET image = '$image' WHERE token = '$token'")->execute();
                ...
                <div class="row">
                    <div class="card overflow-hidden p-0">
                        ...
                        <div class="contact-middle" style="padding: 0;">
                            <form action="edit-post.php?token=<?php echo $token ?>" method="post" enctype="multipart/form-data">
                                <div class="card-body   "><br>
                                    <div class="row">
                                        ...
                                        <div class="col-md-12 mt-4">
                                            <input type="text" name="title" class="form-control form-control-lg" placeholder="title" value="<?php echo $rows['title'] ?>" required>
                                        </div>
                                        <div class="col-md-12 mt-4">
                                            <textarea name="description" class="form-control form-control-lg" placeholder="description" required><?php echo $rows['description'] ?></textarea>
                                        </div>
                                        <div class="col-md-12 mt-4">
                                            <textarea name="address" class="form-control form-control-lg" placeholder="address & contact details" required><?php echo $rows['address'] ?></textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="card-footer">
                                    <button type="button" onclick="location.href = 'myposts.php';" class="btn btn-danger btn-lg pull-left me-2">CANCEL</button>
                                    <button type="submit" name="update_post" class="btn btn-success btn-lg pull-right ms-2">UPDATE</button>
