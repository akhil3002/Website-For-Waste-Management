...
$err = '';
if (isset($_POST['create_post'], $_POST['title'], $_POST['description'])) {
    $token = genToken();
    $title = $_POST['title'];
    $waste_type = $_POST['waste_type'];
    $description = $_POST['description'];
    $address = $_POST['address'];
    $expire_on = $current_date_time_local_plus_2;
    ...
    $db->prepare("INSERT INTO posts (token, title, waste_type, description, address, image, created_by, expire_on) VALUES ('$token', '$title', '$waste_type', '$description', '$address', '$image', '$user', '$expire_on')")->execute();

    $noti_to = 'G4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ';
    $noti_from = trim($_SESSION['SESS_USER_TOKEN']);
    $post_token = $token;
    $notification = 'Someone posted a scrap.'. ' (' . ucwords($title) . '), please check it out for more information.';
    $email_to = '';
                            ...
                            <form action="create-post.php" method="post" enctype="multipart/form-data">
                                <div class="card-body   "><br>
                                    <div class="row">
                                        ...
                                        <div class="col-md-12">
                                            <select name="waste_type" class="form-select w-100" required>
                                                <option value="">choose waste type</option>
                                                <option value="e-waste">E-waste</option>
                                                <option value="Plastic and Scraps">Plastic and Scraps</option>
                                        ...
                                        <div class="col-md-12 mt-4">
                                            <textarea name="description" rows="6" class="form-control form-control-lg" placeholder="description" required></textarea>
                                        </div>
                                        <div class="col-md-12 mt-4">
                                            <textarea name="address" rows="4" class="form-control form-control-lg" placeholder="address & contact details" required></textarea>
                                        </div>
                                        <div class="col-md-12 mt-4">
                                            <input type="file" name="photo" class="form-control form-control-lg" placeholder="image" accept="image/*" required>
                                        </div>
                                    </div>
                                </div>
                                <div class="card-footer">
                                    <button type="button" onclick="location.href = 'myposts.php';" class="btn btn-danger btn-lg pull-left me-2">CANCEL</button>
                                    <button type="submit" name="create_post" class="btn btn-success btn-lg pull-right ms-2">CREATE</button>
                                    <button type="reset" class="btn btn-warning btn-lg pull-right ms-2">RESET</button>
