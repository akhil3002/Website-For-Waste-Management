                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            ...
                            <h3 class="cw">Notifications
                                    <?php
                                    $qry_uncount = $db->prepare("SELECT * FROM notification WHERE noti_to = '$user' AND status = 0");
                                    $qry_uncount->execute();
                                    $nCountUnread = $qry_uncount->rowCount();

                                    $qry_ncount = $db->prepare("SELECT * FROM notification WHERE noti_to = '$user'");
                                    $qry_ncount->execute();
                                    $nCount = $qry_ncount->rowCount();
                                    echo '(' . $nCountUnread . '/' . $nCount . ')';
                                    ?>
                                </h3>
                            </div>
                            <div class="card-body">
                            <div class="list-group">
                                    ...
                                    for ($i = 0; $rows = $qry_ncount->fetch(); $i++) {
                                        ...
                                        $db->prepare("UPDATE notification SET status = 1 WHERE id = '" . $rows['id'] . "'")->execute();

                                    ?>
                                        <a href="post-details.php?token=<?php echo $rows['post_token']; ?>" class="list-group-item col-md-12 <?php echo $status ?>">
                                            <h4 class="list-group-item-heading"><?php echo $rows['notification'] ?></h4>
                                            <p class="list-group-item-text"><?php echo time_convert($rows['created_at']) ?></p>
                                        </a>
                                    <?php } ?>
                                </div>
                            </div>
